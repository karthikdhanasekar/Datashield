...
export default function ScanStatusPage() {
  ...
  useEffect(() => {
    if (scan?.status === 'completed') setAutoRefresh(false)
  }, [scan?.status])
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Live findings preview (top 5 during scan) */}
      {findings.length > 0 && (
        <div className="glass-card p-6">
          <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
            <AlertTriangle className="w-5 h-5 text-orange-400" />
            {isActive ? 'Live Findings' : 'Top Findings'}
          </h2>
          <div className="space-y-3">
            {findings.slice(0, 8).map((f: any) => (
              <div key={f.id} className="flex items-start gap-3 p-3 bg-white/5 rounded-lg">
                <span className={`text-xs font-bold px-2 py-0.5 rounded border flex-shrink-0 mt-0.5 ${SEV_COLORS[f.severity]}`}>
                  {f.severity.toUpperCase()}
                </span>
                <div className="flex-1 min-w-0">
